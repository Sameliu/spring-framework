...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		/**
		 *
		 * <1> 通过 resourcesCurrentlyBeingLoaded.get() 代码，来获取已经加载过的资源
		 *
		 * 然后将 encodedResource 加入其中，如果 resourcesCurrentlyBeingLoaded 中已经存在该资源，则抛出 BeanDefinitionStoreException 异常。
		 *
		 * 为什么需要这么做呢？
		 *
		 * 答案在 "Detected cyclic loading" ，避免一个 EncodedResource 在加载时，还没加载完成，又加载自身，从而导致死循环。
		 *
		...
		/**
		 *
		 * <2> 从 encodedResource 获取封装的 Resource 资源
		 *
		 * 并从 Resource 中获取相应的 InputStream
		 *
		 * 然后将 InputStream 封装为 InputSource
		 *
		 * 最后调用 #doLoadBeanDefinitions(InputSource inputSource, Resource resource) 方法
		 *
	...
	protected int getValidationModeForResource(Resource resource) {
		...
		/**
		 *
		 * <1> 调用 #getValidationMode() 方法，获取指定的验证模式( validationMode )。
		 *
		 * 如果有手动指定，则直接返回。
		 *
		 * 另外，对对于 validationMode 属性的设置和获得的代码
		 */
	...
	protected int detectValidationMode(Resource resource) {
		...
		/**
		 *
		 * 调用 XmlValidationModeDetector#detectValidationMode(InputStream inputStream) 方法，获取相应的验证模式。
		 */
		try {
